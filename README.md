...
PostgreSQL, native go router instead of a third party package, [Mise](https://mise.jdx.dev/) instead of `Make` etc...

It's written as a learning project. Nearly every non-obvious line has a comment
explaining _why_ it's there, not just what it does, and this README explains the
...
[architecture-html]: https://htmlpreview.github.io/?https://github.com/ivandotv/lets-go-further/blob/main/architecture.html
...
of `mise.toml` and run the `go` line directly. Nothing in the project _requires_
...
| -------- | --------------------------- | -------------- | ---------------------------------- |
| `GET`    | `/v1/healthcheck`           | _public_       | Liveness and build info            |
...
| `POST`   | `/v1/users`                 | _public_       | Register (sends activation email)  |
| `PUT`    | `/v1/users/activated`       | _public_       | Activate with an emailed token     |
| `POST`   | `/v1/tokens/authentication` | _public_       | Exchange credentials for a token   |
| `GET`    | `/debug/vars`               | _public_ ⚠️    | expvar runtime metrics             |
...
Every response — success _and_ error — is JSON wrapped in a single top-level
key:
...
- `requestID` is outermost, _including_ outside `recoverPanic` — so a 500 caused
...
- `recoverPanic` sits near the outside so it catches panics from everything
  below it.
- `enableCORS` runs _before_ `rateLimit`, so that a rejected request still gets
...
The previous section follows one request. This one follows the _process_: how it
...
| `<-shutdownError` in `serve()` | Without it, `serve()` returns the moment `Shutdown` is _called_, `run()` closes the database, and the process exits mid-flight. `TestServeGracefulShutdown` asserts exactly this by checking the recorder mailer got its message _after_ `serve()` returned. |

### The timeouts, and why every one is set

The zero value for all of these is _no timeout_, which is how a single slow
...
`ErrorLog` is set too, so the server's _own_ errors (bad TLS handshakes,
...
_Caveat:_ `NOCASE` only folds ASCII A–Z, not full Unicode. Fine for email
...
The book detects a duplicate email by comparing the driver's error _message_:
...
PRAGMAs are **per-connection** settings, and `database/sql` manages a _pool_
...
Putting them in the DSN makes the driver apply them to _every_ connection
(`internal/db/db.go`):
...
We default to 4. The rigorous alternative is _two_ pools — a read pool with N
...
| ----------------------------- | ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `bigserial PRIMARY KEY`       | `INTEGER PRIMARY KEY AUTOINCREMENT` | `AUTOINCREMENT` prevents id _reuse_ after a delete — important for a public API, or a new record could inherit a deleted one's URL. |
| `timestamp(0) with time zone` | `TIMESTAMP`                         | The driver reads the _declared_ type and converts to/from `time.Time` automatically.                                                |
...
`PATCH` is a _partial_ update, so we must distinguish "the client sent `year: 0`"
...
protection the second silently clobbers the first — a _lost update_.
...
`Filters.sortColumn()` checks the value against a safelist the _handler_ sets
...
|                                         | Mechanism                     | Meaning                        |
| --------------------------------------- | ----------------------------- | ------------------------------ |
| Per-request work (welcome email)        | `app.wg`                      | _Wait for this to **finish**._ |
| Long-lived loops (rate-limiter janitor) | `app.shutdown` + `app.stop()` | _Tell this to **stop**._       |
...
once _per test_, so the goroutines pile up and defeat any leak detection.
...
A 500 tells the client only _"the server encountered a problem"_. The actual
...
- **Every PRAGMA reaching every pooled connection** — `internal/db` checks out
  four connections _simultaneously_ and asserts on all of them, which is what
...
assert _properties_ rather than outputs — "never panics", "anything accepted
...
bcrypt at cost 12 takes ~250ms _by design_. A suite that creates dozens of users
...
1.  **`Mailer` is an interface** (declared in `cmd/api/main.go`), not the book's
    ...
    follows the Go idiom of defining an interface where it's _used_, not where
    it's implemented.
...
3.  **Migrations are embedded and applied on startup.** The book runs the
    ...
    against `migrations/` if you prefer.
        ...
        > migrating the _same_ database file at the _same moment_ will wedge it:
    ...
    > `TestMigrateUpAcrossRestarts`. It's a property of golang-migrate's SQLite > driver rather than of this code, so it's documented here rather than > worked around.
